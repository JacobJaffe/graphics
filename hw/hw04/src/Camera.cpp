...
Vector Camera::getScaleVector()
{
  double radians = _viewAngle * PI / 180;
  Vector scale;
  scale[0] = (double(1.0) / (GetScreenWidthRatio() * _farPlane * tan(radians / 2)));
  scale[1] = (double(1.0) / (_farPlane * tan(radians / 2)));
  scale[2] = (double(1.0) / _farPlane);
  return scale;
}

Vector Camera::getTranslationVector()
{
  Vector translation = Vector(_eyePoint[0], _eyePoint[1], _eyePoint[2]);
  return translation;
}
