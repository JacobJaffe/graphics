...
void spline::draw_spline(int resolution, int output){
    ...
    float range = endx - startx;
				for (float i = float(startx); i <= float(endx); i+= range / resolution) {
					float t = (i -startx) / range;
					float y = calculate_Spline(t, a->y, b->y, 10, 5);
					float z = calculate_Spline(t, a->z, b->z, 10, 5);


					glVertex3f(startx + t, y, z);
				}
			}
...
/*	===============================================
		...
		Postcondition:
=============================================== */
float spline::calculate_Spline (float t, float sPos, float gPos, float sVel, float gVel) {
  float value = t*t*t*( 2.0*sPos - 2.0*gPos + 1.0*sVel + 1.0*gVel) +
                  t*t*(-3.0*sPos + 3.0*gPos - 2.0*sVel - 1.0*gVel) +
                    t*(                           sVel       ) +
                      (     sPos);
  return value;
}
